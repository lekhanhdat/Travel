...
> {
  ...
  handleGetReviews = () => {
    const locations: ILocation[] = _.unionBy(LOCATION_POPULAR, LOCATION_NEARLY, 'id');
  ...
  renderItem = ({ item, index }: { item: IReview; index: number }) => {
    return <ReviewItem review={item} isShowLocation={true} />;
  };
      ...
      <Page>
        ...
        <BottomSheet
          ...
          <View style={{ flex: 1 }}>
              ...
              {Array.from([1, 2, 3, 4, 5]).map(i => {
                return (
                  ...
                  >
                    {
                      i <= this.state.star ?
                        <StarActive width={sizes._32sdp} height={sizes._32sdp} color={colors.primary} />
                        :
                        <StarInActive width={sizes._32sdp} height={sizes._32sdp} color={colors.primary_950} />
                    }
                  </TouchableOpacity>
                );
              })}
            ...
            {
              this.state.location && <View style={{ paddingHorizontal: sizes._16sdp }}>
                <LargeItemLocation location={this.state.location} />
              </View>
            }
            <TouchableOpacity
              style={{
                width: sizes.width - sizes._32sdp, marginLeft: sizes._16sdp, height: sizes._40sdp, backgroundColor: colors.primary,
            ...
            <TouchableOpacity
              ...
              style={{
                position: 'absolute', bottom: sizes._32sdp, width: sizes.width - sizes._32sdp, left: sizes._16sdp,
                backgroundColor: (this.state.content.length === 0 || !this.state.location) ? colors.primary_100 : colors.primary,
                alignItems: 'center',
                justifyContent: 'center',
                height: sizes._40sdp,
                borderRadius: sizes._8sdp
          ...
          <View style={{ padding: sizes._16sdp }}>
            <FlatList
              ...
              renderItem={({ item, index }) => {
                return <LargeItemLocation location={item} onPress={() => {
                  this.setState({
                    location: item,
                  }, () => {
