...
### 3.2.3 Sơ đồ Use Case chi tiết - Ứng dụng di động với Extensions

```mermaid
graph TB
    subgraph "Travel App - Detailed Use Cases"
        User((User))
        System((System))
        ...
        %% Location Features
        UC_Location[Location Features]
        UC_ViewList[View Location List]
        UC_ViewDetail[View Location Detail]
        UC_ViewGallery[View Gallery]
        UC_ViewReviews[View Reviews]

        %% Search & AI
        UC_Search[Search Features]
        UC_Semantic[Semantic Search]
        UC_AIChat[AI Chatbot]
        UC_Recommend[Get Recommendations]

        %% Map Features
        UC_Map[Map Features]
        UC_ViewMap[View Map]
        UC_Navigate[Navigate to Location]
        UC_TTS[Text-to-Speech Navigation]

        %% User Actions
        UC_Review[Write Review]
        UC_Donate[Make Donation]
        UC_Profile[Manage Profile]

        User --> UC_Auth
        UC_Auth --> UC_Login
        UC_Auth --> UC_Register
        UC_Auth --> UC_ForgotPwd
        UC_Auth --> UC_Logout

        User --> UC_Location
        UC_Location --> UC_ViewList
        UC_Location --> UC_ViewDetail
        UC_ViewDetail --> UC_ViewGallery
        UC_ViewDetail --> UC_ViewReviews

        User --> UC_Search
        UC_Search --> UC_Semantic
        UC_Search --> UC_AIChat
        UC_Search --> UC_Recommend
...
## 3.7 SƠ ĐỒ TRIỂN KHAI (DEPLOYMENT DIAGRAM)

### 3.7.1 Sơ đồ triển khai hệ thống

```mermaid
graph TB
    subgraph "Client Devices"
        AND[Android Device<br/>React Native App]
        IOS[iOS Device<br/>React Native App]
        WEB[Web Browser<br/>Dashboard Admin]
    end

    subgraph "Cloud Services"
        subgraph "Render.com"
            FAST[FastAPI Server<br/>Port 8000<br/>AI + Search APIs]
            EXPRESS[Express Server<br/>Port 3001<br/>Dashboard APIs]
        end

        subgraph "Vercel"
            VITE[React + Vite<br/>Dashboard Frontend<br/>Static Hosting]
        end

        subgraph "NocoDB Cloud"
            NOCO[(NocoDB<br/>app.nocodb.com<br/>REST API Database)]
        end
    end

    subgraph "Third-party APIs"
        OPENAI[OpenAI API<br/>api.openai.com]
        MAPBOX[Mapbox API<br/>api.mapbox.com]
        PAYOS_API[PayOS API<br/>api.payos.vn]
        SENDGRID[SendGrid API<br/>api.sendgrid.com]
        SERPAPI[SerpAPI<br/>serpapi.com]
    end

    subgraph "Local Storage"
        FAISS_DB[(FAISS Index<br/>Vector Store<br/>Local File)]
    end
    ...
    FAST -->|HTTPS| NOCO
    FAST -->|HTTPS| OPENAI
    FAST -->|HTTPS| PAYOS_API
    FAST -->|HTTPS| SERPAPI
    FAST -->|File I/O| FAISS_DB

    EXPRESS -->|HTTPS| NOCO
    EXPRESS -->|HTTPS| SENDGRID
```

### 3.7.2 Cấu hình môi trường triển khai

```mermaid
graph LR
    subgraph "Development"
        DEV_FAST[FastAPI<br/>localhost:8000]
        DEV_EXPRESS[Express<br/>localhost:3001]
        DEV_VITE[Vite Dev<br/>localhost:5173]
    end

    subgraph "Production"
        PROD_FAST[FastAPI<br/>render.com]
        PROD_EXPRESS[Express<br/>render.com]
        PROD_VITE[Vite Build<br/>vercel.app]
    end

    DEV_FAST -->|Deploy| PROD_FAST
    DEV_EXPRESS -->|Deploy| PROD_EXPRESS
    DEV_VITE -->|Build & Deploy| PROD_VITE
```
